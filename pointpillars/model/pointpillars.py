...
from pointpillars.model.centerpoint.centerpoint_head import CenterHead
from pointpillars.model.centerpoint.centerpoint_targets import build_centerpoint_targets
from pointpillars.model.centerpoint.centerpoint_loss import centerpoint_loss
from pointpillars.model.centerpoint.centerpoint_decode import decode_centerpoint
...
class PointPillars(nn.Module):
    def __init__(
        ...
        # BEV downsample factor:
        # Pillar scatter → 496x432
        # Backbone first stride=2 → 248x216
        self.out_stride = 2
    ...
    def forward(
        ...
        """
        batched_pts: list[tensor], length = batch size
        """

        # 1) Pillarization
        pillars, coors_batch, npoints = self.pillar_layer(batched_pts)

        # 2) Pillar encoder → BEV feature map
        bev = self.pillar_encoder(pillars, coors_batch, npoints)
        # bev: (B, 64, 496, 432)
        ...
        #     print("hm:", preds["hm"].shape)
        #     print("off:", preds["off"].shape)
        #     print("z:", preds["z"].shape)
        #     print("dims:", preds["dims"].shape)
        #     print("rot:", preds["rot"].shape)
        #     self._printed = True
        hm = preds["hm"][0].detach().cpu()
        vis = hm.max(dim=0)[0]   # collapse classes
        ...
        if mode == "train":
            H, W = preds["hm"].shape[-2:]

            targets = build_centerpoint_targets(
                batched_gt_bboxes=batched_gt_bboxes,
                batched_gt_labels=batched_gt_labels,
                feature_map_hw=(H, W),
                voxel_size_xy=(self.voxel_size[0], self.voxel_size[1]),
                pc_range_xy=(
            ...
            loss, loss_dict = centerpoint_loss(preds, targets)
            return loss, loss_dict

        elif mode in ("val", "test"):
            results = decode_centerpoint(
                preds,
                voxel_size_xy=(self.voxel_size[0], self.voxel_size[1]),
                pc_range_xy=(
                    self.point_cloud_range[0],
